...
export default function Sumario() {
  ...
  const { data, profissional, servicos, precoTotal, duracaoTotal, agendar } =
    useAgendamento();
  ...
  function renderizarServico(nome: string, i: number) {
    return (
      <div key={i} className="flex items-center  bg-zinc-700 rounded-md">
        <span
          className={twMerge(
            "text-xs text-zinc-400 px-3 bg-black/25 w-5 py-1.5",
  ...
  return (
    <div className="flex flex-col bg-zinc-950 rounded-lg w-96 lg:w-80">
      ...
      <div className="p-5">
        <button
          className={twMerge(
            "text-sm font-semibold text-zinc-900 w-full py-3 rounded-lg",
            "flex justify-center items-center",
            podeFinalizar() ? "bg-yellow-400" : "bg-zinc-600"
          )}
